...
def main():
    ...
    parser.add_argument('--log-path', type=str, default='log', help='specify path to log folder')
    ...
    train_transform = A.Compose([
                            A.Resize(**dict(height=224, width=128), interpolation=cv.INTER_CUBIC),
...
def val(model,
    ...
    for imgs, gt_kp in loop:
        ...
        ADD, SADD = compute_average_distance(pred_kp, gt_kp)
        # record loss
        ADD_metr.update(ADD, imgs.size(0))
        SADD_metr.update(SADD, imgs.size(0))
        # write to writer for tensorboard
        ...
        loop.set_postfix(ADD=ADD, avr_ADD=ADD_metr.avg, SADD=SADD,
                            avr_SADD=SADD_metr.avg,)
