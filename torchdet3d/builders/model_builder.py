...
from torchdet3d.models import mobilenetv3_large, MobileNetV3
...
def build_model(cfg):
    if cfg.model.name.startswith('efficientnet'):
        ...
        kwargs = dict(blocks_args=blocks_args, global_params=global_params)
        model = model_wraper(model_class=EfficientNet, **kwargs)
        ...
        [5,   6, 160, 1, 1, 1]
            ...
            model = model_wraper(model_class=MobileNetV3, **kwargs)
            if cfg.model.pretrained:
...
def model_wraper(model_class, num_points=18, num_classes=1, pooling_mode='avg', **kwargs):
    class ModelWrapper(model_class):
        ...
        @staticmethod
        def _init_regressors(exp_size, output_channel):
        ...
        def forward(self, x, cats):
            ...
            pooled_features = _glob_feature_vector(features, mode=pooling_mode)
            kp = torch.cat([self.regressors[id](sample) for id, sample in zip(cats, pooled_features)], 0)
            kp = self.sigmoid(kp)
            targets = self.classifier(x) if num_classes > 1 else torch.zeros(kp.size(0), num_classes).to(x.device)
